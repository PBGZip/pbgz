...
class SamVariableBaseLengthFilepipeTest(PBGZTestCase):
    def __init__(self):
        super().__init__("SamVariableBaseLengthFilepipeTest")
        self.source_file = "sam_var_filepipe.sam"
        self.compressed_file = "sam_var_filepipe.sam.pbgz"
        self.decompressed_file = "sam_var_filepipe.sam.dec"
    ...
    def prepare_data(self):
        ...
        base_lengths = [10, 20, 30, 40, 50]
        for i, base_len in enumerate(base_lengths):
            qname = f"VAR_FILE_{i:03d}"
            flag = 0
