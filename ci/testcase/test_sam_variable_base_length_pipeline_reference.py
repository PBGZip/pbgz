"""
测试用例：SamVariableBaseLengthPipelineReferenceTest - 变长基线长度SAM管道测试（带参考基因组）

测试场景：
- 测试通过管道方式处理包含变长基线长度的SAM（带参考基因组）
- 验证复杂管道与参考基因组的组合
- 测试变长序列的高级处理能力

使用命令：
1. cat {input_file} | ./bin/pbgz compress -o {compressed_file} -r {reference_file}
