"""
Test case: FastqIdMismatchReferenceTest - FASTQ file ID mismatch handling test (with reference genome)

Test scenario:
- Test FASTQ files where sequence IDs and quality IDs don't match (with reference genome optimization)
- Verify combination of format fault tolerance and reference genome compression
- Test processing capability of complex parameter combinations
...
class FastqIdMismatchReferenceTest(PBGZTestCase):
    def __init__(self):

        super().__init__("FastqIdMismatchReferenceTest")
        self.source_file = "id_mismatch_ref_test.fq"
        self.compressed_file = "id_mismatch_ref_test.fq.pbgz"
        self.decompressed_file = "id_mismatch_ref_test.fq.dec"
    ...
    def prepare_data(self):
        ...
        for i in range(num_reads):
            seq_id = f"@ID_MISMATCH_REF{i:06d} 1/1"
            
            bases = ''.join(random.choices('ATGCN', k=random.randint(50, 100)))
            
            separator = f"+DIFFERENT{i:06d}"
...
if __name__ == "__main__":
    test_case = FastqIdMismatchReferenceTest()
