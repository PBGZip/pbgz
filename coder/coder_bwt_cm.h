...
public:
    ...
    void encode_line(const uint8_t *in, const int32_t in_len)
    {
        ...
        { /* 未初始化时先初始化 */
            ...
            check_exit(level <= 9 && level >= 0, ERR_INTERNEL, "coder level should in [0, 9], current is %d", level);

            bsize = min(tab[level], BLOCK_SIZE); /* 块大小 */
            len_buf = bsize;
            len_bwt = bsize;
            len_arr = bsize << 2; /* int32_t类型 */
            safe_alloc((len_buf + len_bwt + len_arr), uint8_t, coder_buff);
        ...
        { /* 够一个block时压缩再缓存未压缩部分 */
            ...
            check_exit(idx >= 1, ERR_INTERNEL, "coder transform failed: idx %d", idx);
...
private:
    ...
    int32_t decode_one_block()
    {
        ...
        if (bsize == 0)
            return 0;
        check_exit(bsize > 0, ERR_INTERNEL, "block size %d is invalid", bsize);
        bidx = get32();
        check_exit(bidx >= 1 && bidx <= bsize, ERR_INTERNEL, "corrupt input");
        ...
        {                           /* 没有分配空间，先分配空间 */
            if (bsize >= (1 << 24)) /* 5*N */
            {
                ...
                safe_alloc(i, uint8_t, coder_buff);
