...
#if 0 /*  代码原逻辑默认是手动模式，触发之后会需要手动reset，如果没有reset后面的wait不会阻塞*/
wait_event::wait_event() {
    evt = neosmart::CreateEvent(true, false);
}
#else /*  将默认模式改为自动reset模式 */
wait_event::wait_event() {
    evt = neosmart::CreateEvent(false, false);
}
#endif

wait_event::~wait_event() {
    neosmart::DestroyEvent(evt);
}

void wait_event::reset() {
    neosmart::ResetEvent(evt);
}

void wait_event::wait() {
    neosmart::WaitForEvent(evt);
}

void wait_event::wakeup() {
    neosmart::SetEvent(evt);
}
...
int UnlockedWaitForEvent(neosmart_event_t event, uint64_t milliseconds) {
    ...
    if (!event->State) {
        ...
        do {
            ...
            if (milliseconds != (uint64_t) -1) {
                result = pthread_cond_timedwait(&event->CVariable,
                        &event->Mutex, &ts);
            } else {
...
int WaitForMultipleEvents(neosmart_event_t *events, int count, bool waitAll,
    ...
    while (!done) {
        ...
        if (!done) {
            if (milliseconds != (uint64_t) -1) {
                result = pthread_cond_timedwait(&wfmo->CVariable, &wfmo->Mutex,
                        &ts);
            } else {
